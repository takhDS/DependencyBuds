...
def ego_position(G, ego_target, jitter: float = 0.15, radial_jitter: float = 0.25):
    ...
    ----------
    G: nx.DiGraph
        The full graph with all nodes and edges.
    ego_target: int
        The number of the ego of the network.
    jitter: float
    ...
    -------
    G: nx.ego_graph()
        The graph G containing the ego network for node: ego_target
    pos: dict
    ...
    G = nx.ego_graph(G, n=ego_target, radius=999)
    ...
    distances = nx.shortest_path_length(G, source=ego_target)
